...
static struct graph
   graph_secs = {NULL, NULL, 0, 0, 60, "秒", 1},
   graph_mins = {NULL, NULL, 0, 0, 60, "分", 60},
   graph_hrs  = {NULL, NULL, 0, 0, 24, "小时",   3600},
   graph_days = {NULL, NULL, 1, 0, 31, "天",    86400};
...
struct str *html_front_page(void) {
   ...
   buf = str_make();
   html_open(buf, "图表", /*path_depth=*/0, /*want_graph_js=*/1);

   d_mono = now_mono() - start_mono;
   d_real = now_real() - start_real;
   str_append(buf, "<p>\n");
   str_append(buf, "<b>已测量</b> <span id=\"rf\">");
   ...
   if (strftime(start_when, sizeof(start_when),
      ...
      str_appendf(buf, "<b>, 开始于</b> %s", start_when);

   str_appendf(buf,"<b>.</b><br>\n"
      "<b>侦测到</b> <span id=\"tb\">%'qu</span> <b>字节, "
      "于</b> <span id=\"tp\">%'qu</span> <b>个数据包.</b> "
      "(<span id=\"pc\">%'u</span> <b>个已捕捉,</b> "
      "<span id=\"pd\">%'u</span> <b>个丢失)</b><br>\n"
	  "<b>若开启Turbo ACC硬件加速，图表结果可能不准确</b><br>\n"
      "</p>\n",
      (qu)acct_total_bytes,
