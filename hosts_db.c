...
format_cols_host(struct str *buf)
{
   ...
   if (hosts_db_show_macs) str_append(buf,
      " <th>MAC 地址</th>\n");
   str_append(buf,
      " <th><a href=\"?sort=in\">入站</a></th>\n"
      " <th><a href=\"?sort=out\">出站</a></th>\n"
      " <th><a href=\"?sort=total\">总计</a></th>\n");
   if (opt_want_lastseen) str_append(buf,
      " <th><a href=\"?sort=lastseen\">最近访问</a></th>\n");
   str_append(buf,
      "</tr>\n");
}
...
#define FULL "全屏显示"
   ...
   html_open(buf, "主机列表", /*path_depth=*/1, /*want_graph_js=*/0);
...
static struct str *html_hosts_detail(const char *ip) {
   ...
   str_appendf(buf,
      "<p>\n"
       "<b>主机名:</b> %s<br>\n",
      (h->u.host.dns == NULL)?"(获取中...)":h->u.host.dns);
   ...
   if (h->u.host.last_seen_mono == 0) {
      ...
      if (h->u.host.last_seen_mono <= now_mono()) {
         ...
         str_appendf(buf, " (未来的时间，可能是cpu时钟问题, "
                     "last = %qd, now = %qu)",
